...
export default function AppointmentCard({ appointment, onCancel, onRate, showActions = true }) {
  ...
  return (
    <div className="glass-card p-4 hover:border-white/20 transition-all duration-300">
      ...
      <div className="space-y-2">
        ...
        {type && (
          <span className="inline-block px-2 py-0.5 bg-white/5 rounded-full text-xs text-slate-400 capitalize">{type}</span>
        )}
      </div>
      ...
      {notes && <p className="mt-3 pt-3 border-t border-white/5 text-xs text-slate-500 line-clamp-2">{notes}</p>}
